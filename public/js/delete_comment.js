$(function() {
	$.ajax({
		url: 'http://tdye123.picp.net:12757/checklogin',
		type: 'get',
		dataType: 'json',
		data: '',
		success: function(loginresult) {
			if(!loginresult.havelogan) {
				...
				$.ajax({
					url: 'http://tdye123.picp.net:12757/query/info',
					type: 'get',
					dataType: 'json',
					...
					success: function(info) {
						...
						$.ajax({
							url: 'http://tdye123.picp.net:12757/query/mycomments',
							type: 'get',
							dataType: 'json',
							...
							success: function(commentdata) {
								...
								let background_svg_height = 200 + commentdata.length*124;
								...
								$.ajax({
									url: `http://tdye123.picp.net:12757/query/islike`,
									type: 'get',
									dataType: 'json',
									data: `username=${info.username}`,
									success: function(like_list) {
										...
										for(let i = 0; i < commentdata.length; i ++) {
											if($.inArray(commentdata[i].id, like_list) < 0) {
												html += `<div style="position:relative;over-flow:hidden;font-size:20px; font-family:KaiTi;height:100px;width:1000px;background-color:white;border-radius:30px;border: 2px solid #a1a1a1"><p style="position:relative; margin: 5px 15px 5px 15px">${commentdata[i].text}</p><a href="./comment4more.html?parentnode=${commentdata[i].id}"><img id="${commentdata[i].id}" class="more" src="./images/more.png" style="width:33px;height:33px;position:absolute;right:200px; top:72px;" /></a><img id="${commentdata[i].id}" class="response" src="./images/response.png" style="width:20px;height:20px;position:absolute;right:160px; top:78px;" /><img id="${commentdata[i].id}" class="delete" src="./images/delete.png" style="width:20px;height:20px;position:absolute;right:120px; top:75px;" /><img id="${commentdata[i].id}" class="like" src="./images/like.png" style="width:20px;height:20px;position:absolute;right:75px; top:75px;" /><b style="position:absolute;right:25px; top:75px;">${commentdata[i].like_num}</b><b style="position:absolute;right:250px; top:75px;">${timeConvert(commentdata[i].time)}</b><a href="http://tdye123.picp.net:12757/otherusercenter.html?username=${commentdata[i].username}" style="text-decoration: none;position:absolute;right:475px; top:75px;">${commentdata[i].username}</a></div><div style="height:20px;"></div>`;
											} else {
												html += `<div style="position:relative;over-flow:hidden;font-size:20px; font-family:KaiTi;height:100px;width:1000px;background-color:white;border-radius:30px;border: 2px solid #a1a1a1"><p style="position:relative; margin: 5px 15px 5px 15px">${commentdata[i].text}</p><a href="./comment4more.html?parentnode=${commentdata[i].id}"><img id="${commentdata[i].id}" class="more" src="./images/more.png" style="width:33px;height:33px;position:absolute;right:200px; top:72px;" /></a><img id="${commentdata[i].id}" class="response" src="./images/response.png" style="width:20px;height:20px;position:absolute;right:160px; top:78px;" /><img id="${commentdata[i].id}" class="delete" src="./images/delete.png" style="width:20px;height:20px;position:absolute;right:120px; top:75px;" /><img id="${commentdata[i].id}" class="like" src="./images/liked.png" style="width:20px;height:20px;position:absolute;right:75px; top:75px;" /><b style="position:absolute;right:25px; top:75px;">${commentdata[i].like_num}</b><b style="position:absolute;right:250px; top:75px;">${timeConvert(commentdata[i].time)}</b><a href="http://tdye123.picp.net:12757/otherusercenter.html?username=${commentdata[i].username}" style="text-decoration: none;position:absolute;right:475px; top:75px;">${commentdata[i].username}</a></div><div style="height:20px;"></div>`;
											}
										...
										$("img.like").click(function(e) {
											...
											if($(e.target).attr("src") == "./images/like.png") {
												...
												$.ajax({
													url: `http://tdye123.picp.net:12757/update/likeplus`,
													type: 'get',
													dataType: 'json',
												...
												$(e.target).next().text(parseInt($(e.target).next().text()) - 1);
												$.ajax({
													url: `http://tdye123.picp.net:12757/update/likeminus`,
													type: 'get',
													dataType: 'json',
													data: `username=${info.username}&&id=${comment_id}`,
													success: function(res) {
										...
										$('img.response').click(function(e) {
											...
											$('#reply_area_submit').off().on('click', function() {
												$.ajax({
													url: 'http://tdye123.picp.net:12757/update/reply',
													type: 'post',
													dataType: 'json',
										...
										$('img.delete').off().on('click', function(e) {
											let comment_id = $(e.target).attr("id");
											$($(e.target).parents()).remove();
											let parentnode = 
											$.ajax({
												url: 'http://tdye123.picp.net:12757/update/delete_comment',
												type: 'get',
												dataType: 'json',
												...
												success: function(result) {
													if(result.status == "success") {
														// alert("删除成功！");
														window.location.href = `./delete_comment.html?username=${info.username}`;
													} else {
														alert("删除失败！");
													}
												}
						...
						// $.ajax({
						// 	url: `http://tdye123.picp.net:12757/query/comment`,
						// 	type: 'get',
						// 	dataType: 'json',
						// 	data: `id=${id}`,
						// 	success: function(commentdata) {
						...
						// 				// 	let comment_id = $(e.target).attr("id");
						// 				// 	$.ajax({
						// 				// 		url: 'http://tdye123.picp.net:12757/query/comment4more',
						// 				// 		type: 'get',
						// 				// 		dataType: 'json',
						// 				// 		data: `comment_id=${comment_id}`,
						// 				// 		success: function(commentdata) {
